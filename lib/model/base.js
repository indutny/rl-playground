...
export default class Model {
  ...
  async step(options) {
    ...
    const {
      value: valueTensor,
      newState: newValueStateTensor,
    } = this.forwardValue(
      observation,
      valueStateTensors || this.initialValueState,
      { sampleMask: sampleMaskTensor, rewards: rewardsTensor });

    const value = await valueTensor.data();
    tf.dispose([ valueTensor, rewardsTensor ]);
    ...
    return { newState: newStateTensor, newValueState: newValueStateTensor };
  }
}
