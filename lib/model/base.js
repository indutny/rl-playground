...
export default class Model {
  ...
  sample(observation, state, actionMask, deterministic = false) {
    return tf.tidy(() => {
      const {
        logProbs,
        value,
      ...
      let action;
      if (deterministic) {
        action = tf.argMax(logProbs, -1);
      } else {
        action = tf.multinomial(logProbs, 1).squeeze(-1);
      }
  ...
  async step(options) {
    ...
    const sampleResult = this.sample(
      observation,
      stateTensors || this.initialState,
      actionMask,
      deterministic);
    const {
      action: actionTensor,
