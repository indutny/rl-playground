...
class Tower {
  constructor(name, options) {
    ...
    this.lstm = lstm && lstm.map((units, i) => {
  ...
  build(inputShape) {
    ...
    if (this.lstm) {
      for (const lstm of this.lstm) {
        lstm.build(inputShape);
        inputShape = [ lstm.units ];
      }
    }
    ...
    this.initialState = this.lstm && this.lstm.map((lstm) => {
      return lstm.stateSize.map((size) => {
  ...
  forward(observation, state, training = false) {
    return tf.tidy(() => {
      ...
      if (this.lstm) {
        newState = [];
        for (const [ i, lstm ] of this.lstm.entries()) {
          out = [ out ].concat(state[i]);

          const localNewState = lstm.call(out, {
            training,
          });
          newState.push(localNewState);
          out = localNewState.shift();
        }
      }
  ...
  getLayers() {
    return this.before.concat(this.lstm || []).concat(this.after);
  }
