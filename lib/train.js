...
export default class Train {
  ...
  async loop() {
    ...
    const models = [ this.model ].concat(this.avatars);
    const batons = models.map((model) => {
      return this.envs.map((env) => env.getBaton(model));
    });
    ...
    for (let t = 0; t < this.options.maxSteps; t++) {
      states = await Promise.all(models.map((model, i) => {
        ...
        return this.model.step({
          ...
          history: isMain ? history : undefined,
          rewards: isMain ? rewards : undefined,
        });
      }));
