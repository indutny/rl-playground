...
export default class Train {
  constructor(Model, Environment, options) {
    ...
    this.avatars = [];
    {
      const { generationSpread, totalCount } = this.options.avatar;
      for (let i = 0; i < totalCount; i++) {
        this.avatars.push(new Avatar(Environment, Model, {
          source: this.model,
          generationOff: i * generationSpread,
          generationMod: totalCount * generationSpread,
        }));
      }
    }
  ...
  computeLoss(epochNum, history, metrics) {
  ...
  async runEpoch(epochNum) {
    ...
    const train = await this.optimizer.minimize(() => {
      return this.computeLoss(epochNum, history, metrics);
    }, true);
