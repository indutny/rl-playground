...
export default class Train {
  ...
  async interactOnce({ model, envs, state, history, rewards }) {
    ...
    const {
      ...
      newState,
      value: valueTensor,
    } = this.model.sample(observation, state, actionMask);
    ...
    history.push({
      observation,
      actionMask,
      state: state === model.initialState ? undefined : state,
      action,
  ...
  async interact(model, envs) {
    ...
    for (let t = 0; t < this.options.maxSteps; t++) {
      const newState =
        await this.interactOnce({ model, envs, state, history, rewards });
  ...
  async runEpoch(epochNum) {
    ...
    // Tensors to cleanup
    const { finalRewards, history } = await this.interact(this.model, envs);
