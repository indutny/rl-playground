...
export default class Train {
  constructor(Model, Environment, options) {
    ...
    this.optimizer = this.options.optimizer || tf.train.adam(this.options.lr);
  ...
  async loop() {
    ...
    const initialStates = models.map((model) => undefined);
  ...
  computeLoss(epochNum, model, rewardsToGo, history, metrics) {
    ...
    for (const [ step, entry ] of history.entries()) {
      ...
      // predicted rewards
      let advantage = rewardsToGo[step].sub(tf.tensor(value));
      meanAdvantage = meanAdvantage.add(advantage.mean(-1));

      advantage.sub(advantage.mean(-1));
      advantage.div(advantage.square().mean(-1).relu().sqrt().add(EPSILON));

      // Predict rewards better!
      const liveAdvantage = liveValue.sub(rewardsToGo[step]);
      valueLoss = valueLoss.add(liveAdvantage.square().div(2).mean(-1));

      // Finally, optimize the policy itself
      if (this.options.ppo.enabled) {
        const actionProb = {
    ...
    meanAdvantage = meanAdvantage.div(history.length);
    ...
    const totalRewards = history[0].rewards.slice().fill(0);
    ...
    metrics.set('advantage', meanAdvantage.dataSync()[0]);
  ...
  async runEpoch(epochNum) {
    ...
    debug(`#${epochNum} Loss: ${meanMetrics.get('policy_loss').toFixed(3)} ` +
