...
export default class Train {
  ...
  async loop() {
    ...
    let valueStates = initialStates;

    // Track history only for `mainModel`
    const history = [];
    for (let t = 0; t < this.options.maxSteps; t++) {
      const newStates = [];
      const newValueStates = [];
      for (const [ i, model ] of models.entries()) {
        const isMain = model === mainModel;

        const { newState, newValueState } = await model.step({
          envs: this.envs,
          state: states[i],
          valueState: valueStates[i],

          history: isMain ? history : null,
        });
        newStates.push(newState);
        newValueStates.push(newValueState);
      }
      states = newStates;
      valueStates = newValueStates;
  ...
  computeLoss(epochNum, model, rewardsToGo, history, metrics) {
    ...
    let valueState = model.initialValueState;
    for (const [ step, entry ] of history.entries()) {
      ...
      const past = { sampleMask, rewards: tf.tensor(rewards) };
      const {
        value: liveValue,
        newState: newValueState,
      } = model.forwardValue(observation, valueState, past);
      ...
      // predicted rewards
      const advantage = rewardsToGo[step].sub(tf.tensor(value));

      // Predict rewards better!
      valueLoss = valueLoss.add(
        liveValue.sub(rewardsToGo[step]).square().div(2).mean(-1));
