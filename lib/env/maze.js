...
function generateField(maxSteps) {
  ...
  function walk(x, y, steps = 0) {
    ...
    if (steps >= maxSteps || !top && !bottom && !left && !right) {
  ...
  while (candidates[0].steps > candidates[candidates.length - 1].steps) {
    candidates.pop();
  }

  const { x: finishX, y: finishY, steps: finishSteps } =
...
export default class Environment {
  ...
  static OBSERVATION_DIMS = 12;

  constructor(maxSteps = DEFAULT_MAX_STEPS) {
    this.reset(maxSteps);
  }

  reset(maxSteps = DEFAULT_MAX_STEPS) {
    this.maxSteps = maxSteps;

    this.field = generateField(maxSteps);
    // Copy field
  ...
  ray(x, y, dx, dy) {
    ...
    return { steps, cell: this.field[y][x] };
  }
  ...
  isEmptyOrFinish(x, y) {
    return this.field[y][x] === FINISH_CELL || this.field[y][x] === EMPTY_CELL;
  }
  ...
  observe() {
    ...
    const left = this.ray(x, y, -1, 0);
    const right = this.ray(x, y, 1, 0);
    const top = this.ray(x, y, 0, -1);
    const bottom = this.ray(x, y, 0, 1);

    return [
      left.steps / WIDTH, right.steps / WIDTH,
      top.steps / HEIGHT, bottom.steps / HEIGHT,

      left.cell === FINISH_CELL ? 1 : 0,
      right.cell === FINISH_CELL ? 1 : 0,
      top.cell === FINISH_CELL ? 1 : 0,
      bottom.cell === FINISH_CELL ? 1 : 0,

      this.isVisited(x - 1, y) ? 1 : 0,
      this.isVisited(x + 1, y) ? 1 : 0,
      this.isVisited(x, y - 1) ? 1 : 0,
      this.isVisited(x, y + 1) ? 1 : 0,
    ];
  }
  ...
  actionMask() {
    const { x, y } = this.current;
    return [
      this.isEmptyOrFinish(x, y - 1) ? 1 : 0,
      this.isEmptyOrFinish(x, y + 1) ? 1 : 0,
      this.isEmptyOrFinish(x - 1, y) ? 1 : 0,
      this.isEmptyOrFinish(x + 1, y) ? 1 : 0,
    ];
  }
  ...
  step(action) {
    ...
    return visited ? 0 : 1 / this.maxSteps;
  }
