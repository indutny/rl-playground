...
export default class Environment {
  ...
  observe() {
    ...
    return [
      this.maxDistance(x, y, -1, 0) / WIDTH,
      this.maxDistance(x, y, 1, 0) / WIDTH,
      this.maxDistance(x, y, 0, -1) / HEIGHT,
      this.maxDistance(x, y, 0, 1) / HEIGHT,
  ...
  actionMask() {
    const { x, y } = this.current;
    return [
      this.isEmpty(x, y - 1) ? 1 : 0,
      this.isEmpty(x, y + 1) ? 1 : 0,
      this.isEmpty(x - 1, y) ? 1 : 0,
      this.isEmpty(x + 1, y) ? 1 : 0,
    ];
  }
  ...
  step(action) {
    ...
    return visited ? 1 / (4 * this.maxSteps) : 1 / this.maxSteps;
  }
